...
def calculate_CON_multiorder(df):
    ...
    for date in df['start_date'].unique():
        curr_game = df[df['start_date'] == date]
        ...
        for u in range(n):
            ...
            con_scores.append({'date': date, 'team': players[u], '1st_order_CON_score': con_1st_order, '2nd_order_CON_score': con_2nd_order, 'full_CON_score': con_large})
...
def calculate_CON_multiorder1(df):
    ...
    for date in df['week'].unique():
        curr_game = df[df['week'] == date]
        ...
        for u in range(n):
            ...
            con_scores.append({'date': date, 'team': players[u], '1st_order_CON_score': con_1st_order, 
                               '2nd_order_CON_score': con_2nd_order, 'full_CON_score': con_large,
                               'pagerank': pagerank_score, 'closeness': closeness_score,
                               'in_degree': in_degree, 'out_degree': out_degree, 'betweenness': betweenness_score})
